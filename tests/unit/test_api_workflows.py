...
class Principal:
    def resolve(self, request):
        return AuthenticatedPrincipal("trusted-reviewer")


class NoPrincipal:
    def resolve(self, request):
        raise UnauthenticatedError("private")


def _client(application: Application, principal=None) -> TestClient:
    return TestClient(create_app(application, Readiness(), principal))
...
def test_review_identity_comes_only_from_injected_principal() -> None:
    application = Application()
    response = _client(application, Principal()).post(
...
def test_review_requires_an_authenticated_principal() -> None:
    response = _client(Application(), NoPrincipal()).post(
        "/v1/incidents/LOG_1/reviews",
        json={
            "correlation_id": "review-1",
            "decision_id": "decision-1",
